...
1.벨만 방정식을 이용한 정책 평가 이용>> 우리가 설정한 기준대로 로봇이 움직이면 평균적으로 얼마의 값을 가지는지에 대한 방법<br>
2.벨만 방정식을 이용한 가치 반복 이용>> 무조건 가장 큰 값을 따라갔을때 얼마의 값을 가질 수 있는지에 대한 방법<br>
3.girdworld는 우리가 미리 틀을 정해주고 어느정도 기준을 세워줘야하지만 현실 문제는 그렇지 못함 따라서 이를 바탕으로 Q-Learning과같은 더 복잡한 알고리즘이 필요
